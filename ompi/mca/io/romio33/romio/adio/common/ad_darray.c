                            ...
                            int order, MPI_Datatype oldtype, MPI_Datatype * newtype)
{
    ...
    MPI_Aint *st_offsets, lb, ub, orig_extent, disp;
...
/* nprocs = no. of processes in dimension dim of grid
   rank = coordinate of this process in dimension dim */
    ...
    if (((order == MPI_ORDER_FORTRAN) && (dim == 0)) ||
        ...
        blklens[0] = 1;
        MPI_Type_create_struct(1, blklens, disps, types, &types[1]);
        MPI_Type_create_resized(types[1], 0, orig_extent * (MPI_Aint)array_of_gsizes[dim], &type_tmp);
