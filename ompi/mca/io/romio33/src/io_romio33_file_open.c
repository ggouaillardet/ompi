...
int
mca_io_romio33_file_open (ompi_communicator_t *comm,
                        const char *filename,
                        int amode,
                        opal_info_t *info,
                        ompi_file_t *fh)
...
// isn't ideal but it only happens a few places.
    ...
    data = (mca_io_romio33_data_t *) fh->f_io_selected_data;
//    OPAL_THREAD_LOCK (&mca_io_romio33_mutex);
    ret = ROMIO_PREFIX(MPI_File_open)(comm, filename, amode, ompi_info,
                                      &data->romio_fh);
//    OPAL_THREAD_UNLOCK (&mca_io_romio33_mutex);
...
int
mca_io_romio33_file_close (ompi_file_t *fh)
{
    int ret;
    mca_io_romio33_data_t *data;
    int finalized;
...
int
mca_io_romio33_file_set_size (ompi_file_t *fh,
                            MPI_Offset size)
{
    int ret;
    mca_io_romio33_data_t *data;
...
int
mca_io_romio33_file_preallocate (ompi_file_t *fh,
                               MPI_Offset size)
{
    int ret;
    mca_io_romio33_data_t *data;
...
int
mca_io_romio33_file_get_size (ompi_file_t *fh,
                            MPI_Offset * size)
{
    int ret;
    mca_io_romio33_data_t *data;
...
int
mca_io_romio33_file_get_amode (ompi_file_t *fh,
                             int *amode)
{
    int ret;
    mca_io_romio33_data_t *data;
...
int
mca_io_romio33_file_set_info (ompi_file_t *fh,
                            opal_info_t *info)
{
    int ret;
    mca_io_romio33_data_t *data;
...
int
mca_io_romio33_file_get_info (ompi_file_t *fh,
...
int
mca_io_romio33_file_set_view (ompi_file_t *fh,
                            MPI_Offset disp,
...
int
mca_io_romio33_file_get_view (ompi_file_t *fh,
                            MPI_Offset * disp,
                            struct ompi_datatype_t ** etype,
                            struct ompi_datatype_t ** filetype,
                            char *datarep)
...
int
mca_io_romio33_file_get_type_extent (ompi_file_t *fh,
...
int
mca_io_romio33_file_set_atomicity (ompi_file_t *fh,
                                 int flag)
{
    int ret;
    mca_io_romio33_data_t *data;
...
int
mca_io_romio33_file_get_atomicity (ompi_file_t *fh,
                                 int *flag)
{
    int ret;
    mca_io_romio33_data_t *data;
...
int
mca_io_romio33_file_sync (ompi_file_t *fh)
{
    int ret;
    mca_io_romio33_data_t *data;
...
int
mca_io_romio33_file_seek_shared (ompi_file_t *fh,
                               MPI_Offset offset,
                               int whence)
{
    int ret;
...
int
mca_io_romio33_file_get_position_shared (ompi_file_t *fh,
                                       MPI_Offset * offset)
{
    int ret;
    mca_io_romio33_data_t *data;
...
int
mca_io_romio33_file_seek (ompi_file_t *fh,
                        MPI_Offset offset,
                        int whence)
{
    int ret;
...
int
mca_io_romio33_file_get_position (ompi_file_t *fh,
                                MPI_Offset * offset)
{
    int ret;
    mca_io_romio33_data_t *data;
...
int
mca_io_romio33_file_get_byte_offset (ompi_file_t *fh,
                                   MPI_Offset offset,
                                   MPI_Offset * disp)
{
    int ret;
