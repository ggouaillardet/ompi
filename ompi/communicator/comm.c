...
static int ompi_comm_split_type_get_part (ompi_group_t *group, const int split_type, int **ranks_out, int *rank_size) {
    ...
    for (int i = 0 ; i < size ; ++i) {
        ...
        if (ompi_proc_is_sentinel (proc)) {
            ...
            OPAL_MODEX_RECV_VALUE(ret, PMIX_LOCALITY, &proc_name, &u16ptr, OPAL_UINT16);
            if (OPAL_SUCCESS != ret) {
                continue;
            }
        } else {
                           ...
                           ompi_proc_t ***prprocs )
{
    ...
    for (i=0; i < rsize; i++) {
        ...
        uint16_t *u16ptr, u16;
        u16ptr = &u16;
        OPAL_MODEX_RECV_VALUE(rc, PMIX_LOCALITY, &rprocs[i]->super.proc_name, &u16ptr, OPAL_UINT16);
        if (OPAL_SUCCESS == rc) {
