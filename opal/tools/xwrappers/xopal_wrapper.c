...
main(int argc, char *argv[])
{
    ...
    if (user_data_idx < 0) {
        ...
        opal_show_help("help-xopal-wrapper.txt", "no-options-support", true,
    ...
    if (NULL != options_data[user_data_idx].req_file) {
        ...
        if (0 == strcmp(options_data[user_data_idx].req_file, "not supported")) {
            opal_show_help("help-xopal-wrapper.txt", "no-language-support", true,
        ...
        if (options_data[user_data_idx].req_file[0] != '\0') {
            ...
            if (0 != stat(filename, &buf)) {
                opal_show_help("help-xopal-wrapper.txt", "file-not-found", true,
    ...
    for (i = 0 ; i < user_argc ; ++i) {
        if (0 == strncmp(user_argv[i], "-showme", strlen("-showme")) ||
            ...
            if (0 == strncmp(user_argv[i], "-showme:command", strlen("-showme:command")) ||
                ...
                char * str;
                str = opal_show_help_string("help-xopal-wrapper.txt",
                                            "version", false,
                ...
                char *str;
                str = opal_show_help_string("help-xopal-wrapper.txt", "usage",
                                            false, argv[0],
    ...
    */
    ...
    if (flags & COMP_WANT_PMPI) {
	    opal_show_help("help-xopal-wrapper.txt", "no-profiling-support", true,
		               argv[0], NULL);
    }
#endif
    ...
    if (NULL == exec_argv) {
        opal_show_help("help-xopal-wrapper.txt", "no-compiler-specified", true);
        return 1;
    }
    ...
    if (flags & COMP_DRY_RUN) {
        ...
        if (NULL == tmp) {
            opal_show_help("help-xopal-wrapper.txt", "no-compiler-found", true,
                           exec_argv[0], NULL);
            errno = 0;
            exit_status = 1;
        }  else {
            ...
            if( (OPAL_SUCCESS != ret) || ((0 != exit_status) && (flags & COMP_SHOW_ERROR)) ) {
                ...
                if( OPAL_SUCCESS != ret ) {
                    opal_show_help("help-xopal-wrapper.txt", "spawn-failed", true,
