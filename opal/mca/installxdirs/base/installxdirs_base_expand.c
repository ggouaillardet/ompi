        ...
        if (NULL != (start_pos = strstr(retval, "${" #fieldname "}"))) { \
            ...
            *start_pos = '\0';                                          \
            ...
            asprintf(&retval, "%s%s%s", tmp,                            \
                     opal_install_xdirs.ompiname + destdir_offset,      \
                     end_pos);                                          \
            free(tmp);                                                  \
            changed = true;                                             \
 ...
 */
static char *
opal_install_xdirs_expand_internal(const char* input, bool is_setup)
{
    size_t len, i;
    ...
    /* This is subtle, and worth explaining.
       ...
       Note, too, that this OPAL_DESTXDIR futzing only needs to occur
...
char *
opal_install_xdirs_expand(const char* input)
{
    /* We do NOT want OPAL_DESTDIR expansion in this case. */
    return opal_install_xdirs_expand_internal(input, false);
}


char *
opal_install_xdirs_expand_setup(const char* input)
{
    /* We DO want OPAL_DESTDIR expansion in this case. */
    return opal_install_xdirs_expand_internal(input, true);
}
