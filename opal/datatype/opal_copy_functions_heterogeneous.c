...
opal_dt_swap_long_double(void *to_p, const void *from_p, const size_t size, size_t count, uint32_t remoteArch)
{
    ...
    for (i=0; i<count; i++, to++) {
        if ((opal_local_arch&OPAL_ARCH_LDISINTEL) && !(remoteArch&OPAL_ARCH_LDISINTEL)) {
            ...
            // printf ("128bit float -> 80bit float : negative=%u exponent=%u m0=%u m1=%u m2=%u m3=%u\n", b->negative, b->exponent, b->mantissa0, b->mantissa1, b->mantissa2, b->mantissa3);
            ld.ieee.empty = 0;
            ...
            // printf ("80bit float -> 128bit float: negative=%u exponent=%u m0=%u m1=%u empty=%u pad=%u\n", b->negative, b->exponent, b->mantissa0, b->mantissa1, b->empty, b->pad);
            ld.ieee.mantissa3 = 0;
            ld.ieee.mantissa2 = 0;
